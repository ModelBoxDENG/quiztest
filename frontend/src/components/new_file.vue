	  ...
	  <!--qusetionContainer-->
	  			<div class="questionContainer" v-if="questionIndex<quiz.questions.length" v-bind:key="questionIndex">
	  				...
	  				<div class="optionContainer">
	  					<div class="option" v-for="(response, index) in quiz.questions[questionIndex].responses" @click="selectOption(index)" :class="(userResponses[questionIndex] == index)? (quiz.questions[questionIndex].responses[index].correct == true? 'is-selected_correct':'is-selected_wrong'): ''" :key="index">
   ...
   methods: {
      ...
      selectOption: function(index) {
        this.selectedIndex = index;
         ...
         if (index in userClicked){
            var i_index = userClicked.indexOf(index);
            if (i_index > -1) {
              userClicked.splice(i_index, 1);
            }
         }else{
           userClicked.push(index);
         }
         if (this.numOfCorrect(index)>1){
           this.ifmultiple = true;
         }
         this.ifUserClicked = !this.ifUserClicked;
         //console.log(quiz.questions[index].responses);
         //console.log(this.ifUserClicked);
         //console.log('userClicked',this.userClicked);
         Vue.set(this.userResponses, this.questionIndex, index);
         console.log('userResponses',this.userResponses);
         //console.log(this.lastindex, index);
      ...
      numOfCorrect: function(index){
        ...
        for (let text in quiz.questions[index].responses){
          if (quiz.questions[index].responses[text].correct!=null){
            //console.log(quiz.questions[index].responses[text]);
            num = num+1;
          }
        }
        //console.log(num);
